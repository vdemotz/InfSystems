...
public abstract class JpaDao<K extends Serializable, T extends DomainObject> implements Dao<K, T> {
    ...
    private Predicate[] constructRestrictions(Map<String, Filter> filterMap,
                                              Root <T> entity, CriteriaBuilder cb) {
        if (filterMap == null) {
            return null;
        }
    ...
    private  List<javax.persistence.criteria.Order> constructOrderList( List<OrderFilter> orderList,
                                                                        Root <T> entity, CriteriaBuilder cb) {
        if (orderList == null) {
            return null;
        }
    ...
    private CriteriaQuery<Long> countQueryFromFilterMap(Map<String, Filter> filterMap,
                                                        List<OrderFilter> orderList) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
    ...
    private CriteriaQuery<T> selectQueryFromFilterMap(Map<String, Filter> filterMap,
                                                      List<OrderFilter> orderList) {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<T> query = cb.createQuery(getStoredClass());
        Root <T> entity = query.from(getStoredClass());
